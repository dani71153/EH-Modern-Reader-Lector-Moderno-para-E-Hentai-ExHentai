...
(function() {
  ...
  async function resolveBootstrapData() {
    ...
    bootstrapDataPromise = (async () => {
      // nhentai.xxx prohíbe inyección de scripts en línea bajo CSP estricto; usar directamente el parseo de fallback DOM.
      if (isNhentaiMirrorHost()) {
        return { gallery: null, imageHosts: [] };
      }

      // Primero parsear script en línea; si tiene éxito, no inyectar script de página (evita advertencias CSP en algunos sitios).
      ...
      // Si las variables de la página aún no están listas, no cachear el resultado fallido para permitir reintentos posteriores.
      if (resolved.gallery) {
  ...
  function parseNhentaiXxxData(pathInfo, explicitStartAt) {
    ...
    // Fallback de dirección de página individual: resolver server/dir/id desde el data-src de la imagen actual.
  ...
  async function buildReaderData(startAt) {
    ...
    if (!gallery) {
      // Fallback específico del sitio espejo para no afectar la lógica del sitio principal nhentai.net.
      if (isNhentaiMirrorHost()) {
  ...
  function autoLaunchOnMirrorReaderPage() {
    ...
    // La página /g/{id}/{page}/ entra automáticamente al lector moderno.
